...
%%      testing 
        ...
        %% modefied by lly on 2018.4.14 16:40 
%         max_rois_num_in_gpu = check_gpu_memory(conf, caffe_net);
        max_rois_num_in_gpu = 500;
        ...
        for i = 1:num_images
            ...
            for j = 1:num_classes
                ...
                if ~isempty(inds)
                    ...
                    aboxes{j}{i} = [aboxes{j}{i}; cat(2, single(cls_boxes), single(cls_scores))]; % 将该第j类的boxes和scores保存在aboxes中
                    box_inds{j}{i} = [box_inds{j}{i}; inds];                                      % 第j类的保留的每个boxes和scores所对应的索引
                else
            ...
            %% Visualization added by lly on 2018.4.26 ----------------------------------
            if mod(count, 200) == 0
                ...
                for j = 1:length(boxes_cell)
                    boxes_cell{j} = aboxes_lly{j, 1}{i, 1};
                    boxes_cell{j} = boxes_cell{j}(nms(boxes_cell{j}, 0.3), :);

                    I = boxes_cell{j}(:, 5) > thres;
                    boxes_cell{j} = boxes_cell{j}(I, :);
                end
                figure(i);
%                 set(gcf,'outerposition',get(0,'screensize'));  % full screen display
