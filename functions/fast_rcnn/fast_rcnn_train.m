...
%% training
    ...
    while (iter_ < max_iter)
        ...
        % 第一次调用generate_random_minibatch()函数时，shuffled_inds = [], 根据传入的image_roidb_train(准备好的imdb+rois结合的结构体) conf.ims_per_batch(每次训练使用图片数:2张)，
        % 产生随机的训练数据索引，维度为2x2475 double。其中每一列包含两个张图片索引，做为一个batch的训练图片。此后每次调用，只需要将shuffled_inds的第一列取出赋值给sub_inds做为训练的两张图片即可，
        ...
        % 1×5 cell array
        % [901×600×3×2 single]    [1×1×5×128 single]    [1×1×1×128 single]    [1×1×84×128 single]    [1×1×84×128 single]
        % 两张图片  128个rois(每张图片64个rois[16前景48背景])  128个labels   128个rois需要做的平移尺度变换(4维)+rois索引(1维)   标记每一个rois是否属于某一个类
...
function [shuffled_inds, sub_inds] = generate_random_minibatch(shuffled_inds, image_roidb_train, ims_per_batch)
    ...
    if isempty(shuffled_inds)
        ...
        % random perm
        lim = floor(length(hori_image_inds) / ims_per_batch) * ims_per_batch;       % lim = 4064
        hori_image_inds = hori_image_inds(randperm(length(hori_image_inds), lim));  % 4064x1 double
        lim = floor(length(vert_image_inds) / ims_per_batch) * ims_per_batch;       % lim = 886
        vert_image_inds = vert_image_inds(randperm(length(vert_image_inds), lim));  % 886x1  double
        
        % combine sample for each ims_per_batch 
        hori_image_inds = reshape(hori_image_inds, ims_per_batch, []);              % 2x2032 double
        vert_image_inds = reshape(vert_image_inds, ims_per_batch, []);              % 2x443  double
        
        shuffled_inds = [hori_image_inds, vert_image_inds];                         % 2x2475 double
        shuffled_inds = shuffled_inds(:, randperm(size(shuffled_inds, 2)));
        
        shuffled_inds = num2cell(shuffled_inds, 1); % 1x2475 cell
    end
...
%%  try to train/val with images which have maximum size potentially, to validate whether the gpu memory is enough  
    ...
    im_blob = single(zeros(max(conf.scales), conf.max_size, 3, conf.ims_per_batch));                % 4-D single: 600x1000x3x2
    rois_blob = single(repmat([0; 0; 0; max(conf.scales)-1; conf.max_size-1], 1, conf.batch_size)); % 5x128 single
    rois_blob = permute(rois_blob, [3, 4, 1, 2]);       % 4-D single: 1x1x5x128
    labels_blob = single(ones(conf.batch_size, 1));     % 128x1 single
    labels_blob = permute(labels_blob, [3, 4, 2, 1]);   % 4-D single: 1x1x1x128
    bbox_targets_blob = zeros(4 * (num_classes+1), conf.batch_size, 'single'); % 84x128 single
    bbox_targets_blob = single(permute(bbox_targets_blob, [3, 4, 1, 2]));      % 4-D single: 1x1x84x128
    bbox_loss_weights_blob = bbox_targets_blob;                                % 4-D single: 1x1x84x128
    
    net_inputs = {im_blob, rois_blob, labels_blob, bbox_targets_blob, bbox_loss_weights_blob};
    % 1×5 cell array: [600×1000×3×2 single]    [1×1×5×128 single]    [1×1×1×128 single]    [1×1×84×128 single]    [1×1×84×128 single]
