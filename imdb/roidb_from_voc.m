...
if isfield(voc_rec, 'objects')
  ...
  gt_boxes = cat(1, voc_rec.objects(valid_objects).bbox); % 获取每张图片的ground truth
  if flip % 是否水平翻转
    gt_boxes(:, [1, 3]) = voc_rec.imgsize(1) + 1 - gt_boxes(:, [3, 1]);
  end
  all_boxes = cat(1, gt_boxes, boxes); % 把该张图片的ground truth放到其boxes的最前面，组成all_boxes 
  gt_classes = class_to_id.values({voc_rec.objects(valid_objects).class}); % 获取改张图片中每个ground truth所属类别
  gt_classes = cat(1, gt_classes{:});
  num_gt_boxes = size(gt_boxes, 1); % 该张图片中ground truth个数
else
  gt_boxes = [];
  all_boxes = boxes;
  gt_classes = [];
...
num_boxes = size(boxes, 1); % 该张图片使用Selective search方法产生的boxes个数

rec.gt = cat(1, true(num_gt_boxes, 1), false(num_boxes, 1)); % gt中存放的是1和0，分别表示该张图片中所有的boxes(all_boxes)，哪些是ground truth，哪些是Selective search 产生的boxes.
rec.overlap = zeros(num_gt_boxes+num_boxes, class_to_id.Count, 'single'); % 用于存放该张图片使用Selective Search产生的boxes与每个ground truth的overlap重叠率，并保留在ground truth所属类别的那一列。如果有一个boxes与两个属于同一类别的ground truth计算overlap，我们只保留其最高的overlap在该列
for i = 1:num_gt_boxes
  rec.overlap(:, gt_classes(i)) = ...
      max(rec.overlap(:, gt_classes(i)), boxoverlap(all_boxes, gt_boxes(i, :)));
end
rec.boxes = single(all_boxes); % rec.boxes存放all_boxes:[ground truth boxes + Selective Search boxes] 
rec.feat = [];
