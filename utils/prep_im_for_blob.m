%% [im, im_scale] = prep_im_for_blob(im, im_means, target_size, max_size)
% Using prep_im_for_blob_size() function to get a image im_scale, and then
% use this in_scale to resize this image.
function [im, im_scale] = prep_im_for_blob(im, im_means, target_size, max_size)
    ...
    if ~isa(im, 'gpuArray')
        ...
        % for im as gpuArray
        try
            im = bsxfun(@minus, im, im_means); % minus equal Subtraction. 
        catch
        ...
        im_scale = prep_im_for_blob_size(size(im), target_size, max_size); % #lly# see function detail 
